...
const AGENTS_BASE = [
  ...
  { id: "gulf", name: "Gulf Remit FZ", category: "Payout partners", relationship: "UAE corridor payout", jur: "UAE", juris: ["UAE"], license: "CBUAE-registered; ADGM contracting", tier: "High", live: false,
...
const DAY = 864e5;
async function crLoad() { try { if (window.storage) { const r = await window.storage.get(CR_KEY); if (r && r.value) return JSON.parse(r.value); } } catch (e) {} return null; }
async function crSave(o) { try { if (window.storage) await window.storage.set(CR_KEY, JSON.stringify(o)); } catch (e) {} }
...
function ControlRegister() {
  ...
  const setRow = (id, patch) => { const cur = rows.find(r => r.id === id); const next = { status: cur.status, lastTested: cur.lastTested, evidence: cur.evidence, ...patch }; setOv(o => { const no = { ...o, [id]: next }; crSave(no); return no; }); };
...
async function cmLoad() { try { if (window.storage) { const r = await window.storage.get(CM_KEY); if (r && r.value) return JSON.parse(r.value); } } catch (e) {} return null; }
async function cmSave(o) { try { if (window.storage) await window.storage.set(CM_KEY, JSON.stringify(o)); } catch (e) {} }
...
function CaseManagement() {
  ...
  const openCaseFromAlert = (al) => { const seq = data.seq + 1; const id = "CASE-2026-" + String(seq).padStart(4, "0"); const tCtl = (THREAT_ATLAS.typologies.find(t => t.id === al.typology)?.def) || ["TM-1"]; const nc = { id, title: al.typName + " — " + al.corridor, typology: al.typology, typName: al.typName, sev: al.sev, source: al.source, cust: al.cust, corridor: al.corridor, amount: al.amount, status: "Open", assignee: "", opened: cmNow(), controls: tCtl, disposition: "", rationale: "", reviewer: "", approvedAt: null, sarClockStart: null, sarFiled: null, narrative: "", dpl: [{ at: cmNow(), who: "System", act: "Case opened", note: "Promoted from " + al.source + " alert " + al.id + ": " + al.signal }] }; persist({ ...data, cases: [nc, ...cases], alerts: alerts.filter(a => a.id !== al.id), seq }); setSub("cases"); setSelId(id); };
  const addDpl = () => { if (!dplNote.trim()) return; upd(sel.id, { status: sel.status === "Open" ? "Investigating" : sel.status }, { at: cmNow(), who: "You", act: dplAct, note: dplNote.trim() }); setDplNote(""); };
  const propose = () => { if (!dispWhy.trim()) return; upd(sel.id, { status: "Pending QA", disposition: dispChoice, rationale: dispWhy.trim(), sarClockStart: dispChoice === "SAR" && !sel.sarClockStart ? cmNow() : sel.sarClockStart }, { at: cmNow(), who: "You", act: "Disposition proposed", note: dispChoice + " — " + dispWhy.trim() }); };
  const approve = () => { const isSar = sel.disposition === "SAR"; upd(sel.id, { status: isSar ? "Pending QA" : sel.disposition === "Escalate" ? "Escalated" : "Closed — No SAR", reviewer: reviewerRole, approvedAt: cmNow() }, { at: cmNow(), who: reviewerRole, act: "Four-eyes approved", note: "Disposition " + sel.disposition + " approved (maker-checker)." }); };
  const fileSar = () => { const sid = "SAR-2026-" + String(7 + sars.length).padStart(4, "0"); const next = { ...data, cases: cases.map(c => c.id === sel.id ? { ...c, status: "SAR filed", sarFiled: cmNow(), narrative: sarNarr || c.narrative, dpl: [...c.dpl, { at: cmNow(), who: "You", act: "SAR filed", note: "Filed via BSA E-Filing (" + sid + ")." }] } : c), sars: [{ id: sid, case: sel.id, cust: sel.cust, typName: sel.typName, filed: cmNow(), amount: sel.amount }, ...sars] }; persist(next); };
...
async function rtLoad() { try { if (window.storage) { const r = await window.storage.get(RT_KEY); if (r && r.value) return JSON.parse(r.value); } } catch (e) {} return null; }
async function rtSave(o) { try { if (window.storage) await window.storage.set(RT_KEY, JSON.stringify(o)); } catch (e) {} }
...
function RegChange() {
  ...
  const setTask = (id, status) => setRt(o => { const no = { ...o, [id]: status }; rtSave(no); return no; });
