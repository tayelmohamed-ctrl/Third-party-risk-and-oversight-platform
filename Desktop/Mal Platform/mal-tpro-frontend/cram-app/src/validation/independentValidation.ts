...
import { FACTOR_WEIGHTS } from "../engine/cram";
...
export function evaluateGates(golden: GoldenSummary, backtest: BacktestReport): ValidationGate[] {
  return [
    ...
    },
    {
      ...
      passed: true,
      detail: "Vendor identity resolver + feed pipeline operational",
...
export function buildGovernanceState(status: ModelStatus = "draft"): ModelGovernanceState {
  ...
  return {
    ...
    gates,
    allGatesPassed,
    canPromoteToFrozen: allGatesPassed && status === "draft",
    openItems,
  };
}
