...
import { statusLabel, statusPillClass } from "../config/openItemsRegister";
import {
  apiValidationGovernance, apiPromoteModel, apiRunValidation,
  apiOpenItems, type ValidationGovernance, type OpenItemRow,
} from "../lib/api";

export default function Governance() {
  const [gov, setGov] = useState<ValidationGovernance | null>(null);
  const [openItems, setOpenItems] = useState<OpenItemRow[]>([]);
  const [itemCounts, setItemCounts] = useState({ open: 0, accepted: 0, corrected: 0, total: 0 });
  const [busy, setBusy] = useState(false);
  const [msg, setMsg] = useState<string | null>(null);

  const refresh = useCallback(async () => {
    try {
      const [g, oi] = await Promise.all([apiValidationGovernance(), apiOpenItems()]);
      setGov(g);
      setOpenItems(oi.items);
      setItemCounts(oi.counts);
    } catch {
      setGov(null);
    }
  }, []);
      ...
      <div className="grid grid-cols-2 gap-4 max-md:grid-cols-1">
        ...
        <Card className="p-4">
          ...
          <div className="text-[10px] text-muted mb-3">
            MLRO open items register (DB-backed) ·
