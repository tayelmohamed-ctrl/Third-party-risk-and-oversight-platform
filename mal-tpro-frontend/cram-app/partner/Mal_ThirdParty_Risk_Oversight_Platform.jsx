...
import {
  REPORT_TYPES, DD_ITEMS, REG_DEF, emptyRegisters, normalizeStore,
  registerKeyForReport, REPORT_FORM_SCHEMA, reportFormReady, buildReportSummary,
  schemaFor, PARTNER_DD_PACK, downloadPartnerSchemaPack,
} from "./partnerComplianceConfig.js";
...
const PARTNER_PORTAL = {
  "Payout partners": {
    ...
    quickLinks: [
      ...
      { tab: "reportingA", label: "Filings & certifications", desc: "STR/SAR, interdiction, subpoena, periodic attestation, certifications", icon: FileText },
...
async function loadFcieLocal() { try { if (window.storage) { const r = await window.storage.get(KEY_FCIE); if (r && r.value) return JSON.parse(r.value); } } catch (e) {} return null; }
...
const FEED_KINDS = ["SAR", "STR", "CTR", "Filing", "Legal process", "Certification", "Audit", "Correspondence", "Policy update", "Risk assessment", "Training", "Corrective action"];
...
async function loadFcie() {
  try {
    if (await apiAvailable()) {
      const data = await apiFetch("/intelligence/feed?limit=200", { auth: false });
  ...
  AGENTS_BASE.forEach((a, i) => {
    agents[a.id] = {
      ...
      registers: {
        ...
        cert: [{ ref: "CERT-2026", at: daysFrom(-40), status: "Submitted", summary: "Annual compliance certification 2026" }],
        correspondence: [{ ref: "RC-" + a.id + "-1", at: daysFrom(-20), status: "Closed", summary: "Regulator query on corridor volumes" }],
        periodic_review: i % 3 === 0 ? [{ ref: "PR-" + a.id + "-2026Q1", at: daysFrom(-30), status: "Submitted", summary: "Q1 2026 periodic compliance attestation — programme effective" }] : [],
      },
...
// LIGHT intake schemas — see partnerComplianceConfig.js (shareable partner DD pack)
...
function Onboarding({ store, category, setCategory, onboard, viewProfile }) {
  ...
  return (
    <>
      ...
      <p className="sub">Partnership, not interrogation. Partners complete a shareable intake covering entity data, MLRO contacts, STR/SAR capability, sanctions interdiction, subpoena notification, and periodic programme review — Mal runs independent verification and screening on top.</p>
      <div className="card" style={{ marginBottom: 16, borderLeft: "3px solid var(--gold)" }}>
        <h3 style={{ marginTop: 0 }}>{PARTNER_DD_PACK.title}</h3>
        <p className="muted" style={{ fontSize: 12.5 }}>{PARTNER_DD_PACK.summary}</p>
        <div className="muted" style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: ".5px", margin: "10px 0 6px" }}>Ongoing monitoring registers (partner submissions)</div>
        <div className="row" style={{ gap: 6, flexWrap: "wrap", marginBottom: 10 }}>{PARTNER_DD_PACK.monitoringRegisters.map(r => <span key={r} className="chip" style={{ background: "var(--elevated)" }}>{r}</span>)}</div>
        <div className="muted" style={{ fontSize: 11.5, textTransform: "uppercase", letterSpacing: ".5px", margin: "8px 0 6px" }}>Contractual SLA highlights</div>
        <ul style={{ margin: "0 0 12px", paddingLeft: 18, fontSize: 12.5 }}>{PARTNER_DD_PACK.slaHighlights.map((s, i) => <li key={i} style={{ marginBottom: 4 }}>{s}</li>)}</ul>
        <button className="btn gold" onClick={() => downloadPartnerSchemaPack(null)}><Download size={14} /> Download partner DD pack (JSON schema)</button>
      </div>
      ...
      <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))" }}>
        {CATEGORIES.map(cat => (
          <div className="card" key={cat.id}>
            ...
            <button className="btn ghost" style={{ marginTop: 8 }} onClick={() => downloadPartnerSchemaPack(cat.id)}><Download size={13} /> Export {cat.id} schema</button>
          </div>
        ))}
      </div>
      ...
      <div className="card" style={{ marginBottom: 13 }}><h3>{supervisor ? "Partner-provided intake (minimum data)" : "Your submitted details"}</h3>
        {ob ? (
          <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              <div className="muted" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".5px", margin: "8px 0 4px" }}>MLRO & legal process contacts</div>
              <DDRow label="MLRO / BSA Officer" v={ob.mlroName ? ob.mlroName + (ob.mlroEmail ? " · " + ob.mlroEmail : "") : ""} />
              <DDRow label="Legal process contact" v={ob.legalProcessContact} />
            </div>
            <div style={{ gridColumn: "1 / -1" }}>
            ...
            <div style={{ gridColumn: "1 / -1" }}>
              ...
              <div className="muted" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: ".5px", margin: "12px 0 4px" }}>Regulatory reporting & monitoring</div>
              <DDRow label="STR/SAR filing capability" v={ob.strCapability} /><DDRow label="US CTR capability (if applicable)" v={ob.ctrCapability} />
              <DDRow label="Subpoena / legal process notification (≤10 bd)" v={ob.subpoenaNotifyAttest} /><DDRow label="Sanctions interdiction procedures" v={ob.interdictionAttest} />
              <DDRow label="314(a) response capability (US nexus)" v={ob.a314Capable} /><DDRow label="Periodic review cadence" v={ob.periodicReviewCadence} />
              <DDRow label="Last programme review" v={ob.lastProgrammeReview} /><DDRow label="Last independent AML audit" v={ob.lastIndependentAudit} />
...
function RegChange() {
  ...
  return (<>
    ...
    <div className="card" style={{ marginTop: 13 }}><div className="row" style={{ gap: 8, alignItems: "flex-start" }}><AlertCircle size={14} className="muted" style={{ marginTop: 1 }} /><span className="muted" style={{ fontSize: 11.5 }}>This is the <b>governed, source-cited</b> record that drives remediation — distinct from the live online ticker, which is unverified horizon-scanning. Entries are a curated demonstrator seed; dates marked [VERIFY] are time-sensitive. In production this is fed by official FinCEN / OFAC / FATF / regulator feeds and each item is confirmed at source before a task is raised.</span></div></div>
  </>);
}
...
export default function App() {
  ...
  useEffect(() => { (async () => { const s = await loadStore(); setStore(s && s.v === SEED_VERSION ? normalizeStore(s) : seed()); const c = await loadLive(); if (c && c.items?.length) setLive({ ...c, status: "ok" }); const f = await loadFcie(); if (f && f.items?.length) setFcie({ ...f, status: "ok" }); })(); }, []);
  ...
  const fetchFCIE = useCallback(async () => {
    ...
    try {
      const res = await fetch("/tpro-api/intelligence/feed?limit=200", { signal: AbortSignal.timeout ? AbortSignal.timeout(5000) : undefined });
      if (res.ok) {
        const data = await res.json();
        const items = (data.items || []).filter(i => FCIE_JURISDICTIONS.includes(i.country));
        const next = { items, at: new Date().toISOString(), status: "ok" };
  ...
  function submitReport(agentId, typeId, values) {
    ...
    const ref = `${t.name.split(" ")[0].toUpperCase().replace(/[^A-Z0-9]/g, "")}-${agentId}-${String(Date.now()).slice(-4)}`;
    const summary = buildReportSummary(typeId, values);
    const rec = { ref, at: new Date().toISOString(), status: "Filed", summary, subject: values.subject || "", period: values.period || "", meta: { ...values } };
    const regKey = registerKeyForReport(typeId);
    const feedKind = typeId === "sar" ? "SAR" : typeId === "str" ? "STR" : typeId === "ctr" ? "CTR" : typeId === "subpoena" ? "Legal process" : "Filing";
    setStore(prev => {
      const ag = { ...prev.agents[agentId] };
      ag.registers = { ...ag.registers, [regKey]: [rec, ...(ag.registers[regKey] || [])] };
      ag.feed = [{ id: "f" + Date.now(), kind: feedKind, at: rec.at, text: `${t.name} submitted — ${rec.summary}`, actor: partnerName(agentId) }, ...ag.feed];
...
const kindIcon = (k) => { const m = { SAR: "🚩", STR: "🚩", CTR: "💵", Filing: "📄", "Legal process": "⚖️", Certification: "✔️", Audit: "🔍", Correspondence: "✉️", "Policy update": "📕", "Risk assessment": "📊", Training: "🎓", "Corrective action": "🛠️" }; return <span style={{ fontSize: 15 }}>{m[k] || "•"}</span>; };
const feedColor = (k) => ({ SAR: "var(--red)", STR: "var(--red)", CTR: "var(--red)", "Legal process": "var(--violet)", Audit: "var(--violet)", Training: "var(--green)", "Policy update": "var(--blue)" }[k] || "var(--gold2)");
...
function AgentReporting({ store, agentId, onSubmit }) {
  ...
  return (
    <>
      ...
      <p className="sub">Submit STR/SAR, CTR, sanctions interdiction, OFAC blocks, 314(a), subpoena notifications, periodic compliance attestations, and certifications. Each submission is versioned, timestamped, added to your register, and synced to the supervisor dashboard for programme review.</p>
      ...
      <div className="card" style={{ marginTop: 13 }}><h3>High-risk partner alerts</h3>
        {agents.filter(({ base }) => ["High", "Critical"].includes(ratingFor(overall(base)).t)).map(({ base, slice }) => { const ew = earlyWarnings(base, slice, store.reviews); return (
          <div className="row" key={base.id} style={{ justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #e7e9f2" }}>
            <span><AlertCircle size={14} color="var(--red)" style={{ verticalAlign: "-2px", marginRight: 7 }} /><b>{base.name}</b> <span className="muted">· {base.jur} · score {overall(base)}</span></span>
...
function ReportModal({ type, onClose, onSubmit }) {
  const [v, setV] = useState({});
  const schema = REPORT_FORM_SCHEMA[type.id];
  const ready = reportFormReady(type.id, v);
  const renderExtra = (f) => {
    if (f.t === "area") return <div className="field" key={f.k}><label>{f.label}{f.required ? " *" : ""}</label><textarea value={v[f.k] || ""} onChange={e => setV({ ...v, [f.k]: e.target.value })} /></div>;
    if (f.t === "select") return <div className="field" key={f.k}><label>{f.label}{f.required ? " *" : ""}</label><select value={v[f.k] || ""} onChange={e => setV({ ...v, [f.k]: e.target.value })}><option value="">Select…</option>{f.options.map(o => <option key={o}>{o}</option>)}</select></div>;
    if (f.t === "yn") return <div className="field" key={f.k}><label>{f.label}{f.required ? " *" : ""}</label><select value={v[f.k] || ""} onChange={e => setV({ ...v, [f.k]: e.target.value })}><option value="">Select…</option><option value="Y">Yes</option><option value="N">No</option></select></div>;
    if (f.t === "date") return <div className="field" key={f.k}><label>{f.label}{f.required ? " *" : ""}</label><input className="input" type="date" value={v[f.k] || ""} onChange={e => setV({ ...v, [f.k]: e.target.value })} /></div>;
    return <div className="field" key={f.k}><label>{f.label}{f.required ? " *" : ""}</label><input className="input" value={v[f.k] || ""} onChange={e => setV({ ...v, [f.k]: e.target.value })} /></div>;
  };
  return <Modal title={"Submit " + type.name} sub={type.reg + " · versioned, timestamped, audit-logged · structured fields support periodic programme review"} onClose={onClose}
    footer={<><button className="btn ghost" onClick={onClose}>Cancel</button><button className="btn gold" disabled={!ready} onClick={() => onSubmit(v)}><Send size={14} /> Submit</button></>}>
    ...
    <div className="field"><label>Summary (required)</label><textarea value={v.summary || ""} onChange={e => setV({ ...v, summary: e.target.value })} placeholder="Brief narrative for the register and supervisor dashboard" /></div>
    <div className="field"><label>Reporting period</label><input className="input" value={v.period || ""} onChange={e => setV({ ...v, period: e.target.value })} placeholder="e.g. Q1 2026, Jan–Mar 2026" /></div>
    {schema?.extra?.map(renderExtra)}
  </Modal>;
}
