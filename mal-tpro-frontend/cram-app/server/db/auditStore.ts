...
import { buildLibraryVersionsSnapshot, getActiveModelVersionId, boundarySetLabel } from "../../src/config/modelProvenance";
...
function rowToAssessment(r: {
  ...
  modelVersionId?: string | null; boundarySetUsed?: string | null; libraryVersions?: unknown;
}): Assessment {
  return {
    ...
    modelVersionId: r.modelVersionId ?? undefined,
    boundarySetUsed: r.boundarySetUsed ?? undefined,
    libraryVersions: (r.libraryVersions ?? undefined) as Assessment["libraryVersions"],
  };
}

function stampAssessmentProvenance(a: Assessment): Assessment {
  return {
    ...a,
    modelVersionId: a.modelVersionId ?? getActiveModelVersionId(),
    boundarySetUsed: a.boundarySetUsed ?? boundarySetLabel(a.boundary),
    libraryVersions: a.libraryVersions ?? buildLibraryVersionsSnapshot(),
  };
}

export async function appendAssessment(a: Assessment) {
  const stamped = stampAssessmentProvenance(a);
  await prisma.assessment.create({
    data: {
      id: stamped.id, customerId: stamped.customerId, customerName: stamped.customerName,
      at: new Date(stamped.at), trigger: stamped.trigger, triggerNote: stamped.triggerNote ?? null,
      rating: stamped.rating, prevRating: stamped.prevRating ?? null, composite: stamped.composite,
      mathBand: stamped.mathBand, boundary: stamped.boundary,
      overrides: stamped.overrides as object, reviewDue: new Date(stamped.reviewDue),
      actor: stamped.actor, input: stamped.input as object,
      governance: stamped.governance ? (stamped.governance as object) : null,
      handoff: stamped.handoff ? (stamped.handoff as object) : null,
      overrideJustification: stamped.overrideJustification ?? null,
      modelVersionId: stamped.modelVersionId ?? null,
      boundarySetUsed: stamped.boundarySetUsed ?? null,
      libraryVersions: stamped.libraryVersions ? (stamped.libraryVersions as object) : null,
    },
  });
  await appendAudit({
    actor: stamped.actor, action: "assessment.appended", entity: "assessment", entityId: stamped.id,
    detail: `${stamped.customerName} (${stamped.customerId}) · ${stamped.trigger} · ${stamped.prevRating ?? "—"} → ${stamped.rating} · model ${stamped.modelVersionId}`,
    before: stamped.prevRating, after: stamped.rating,
  });
  if (stamped.governance) {
    await appendAudit({
      actor: stamped.governance.approvedBy,
      action: "override.applied",
      entity: "assessment",
      entityId: stamped.id,
      detail: `MLRO override ${stamped.governance.requestedBand} · math=${stamped.governance.mathBand} · floor=${stamped.governance.floor ?? "none"} · ${stamped.governance.justification.slice(0, 120)}`,
      before: stamped.mathBand,
      after: stamped.rating,
    });
  }
  if (stamped.handoff?.ops?.deployed) {
    await appendAudit({
      actor: stamped.actor,
      action: "monitoring.deployed",
      entity: "assessment",
      entityId: stamped.id,
      detail: `TM profile deployed · ${stamped.handoff.monitoringIntensity} · ${stamped.handoff.disposition}`,
      after: stamped.handoff.monitoringIntensity,
    });
  }
}
