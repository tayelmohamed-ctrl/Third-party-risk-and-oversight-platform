...
import { openItemCountsFromDb } from "../governance/openItemsStore";
...
export async function getModelGovernance(): Promise<ModelGovernanceState> {
  ...
  const registerCounts = await openItemCountsFromDb();
  return buildGovernanceState(status, registerCounts);
}
...
export async function promoteModelToFrozen(actor: string): Promise<{ ok: true; state: ModelGovernanceState } | { ok: false; error: string; state: ModelGovernanceState }> {
  ...
  if (state.registerCounts.open > 0) {
    return { ok: false, error: `Promotion blocked — ${state.registerCounts.open} open item(s) on register`, state };
  }
  ...
  return { ok: true, state: buildGovernanceState("frozen", state.registerCounts) };
}
