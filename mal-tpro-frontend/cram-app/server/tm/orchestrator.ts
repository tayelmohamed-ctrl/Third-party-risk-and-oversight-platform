...
import { createCaseFromTmAlert, warrantsAutoCase } from "../investigations/orchestrator";
import { maybeRegisterCtrFromTmAlert } from "../ctr/orchestrator";
...
export async function handleOscilarWebhook(payload: OscilarWebhookPayload): Promise<{
  ...
  if (normalized.amount != null && normalized.amount >= 10_000) {
    await maybeRegisterCtrFromTmAlert({
      tmAlertId: alert.id,
      customerId: normalized.customerId,
      customerName: normalized.customerName,
      amount: normalized.amount,
  ...
  const finalAlert = await findTmAlertByOscilarId(normalized.alertId);
  const alertForCase = finalAlert ?? alert;
  let investigationCaseId: string | null = null;
  if (warrantsAutoCase(normalized.severity)) {
    const invCase = await createCaseFromTmAlert(alertForCase);
    investigationCaseId = invCase?.id ?? null;
  }

  const updated = await findTmAlertByOscilarId(normalized.alertId);
  return {
    ok: true,
    alert: updated ?? alertForCase,
    investigationCaseId,
  };
}
